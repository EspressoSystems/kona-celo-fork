...
pub struct HardForkConfig {
    ...
    /// `espresso_enforcement_time` sets the activation time for the Espresso event-only batch
    /// authorization enforcement. This is an Espresso-specific, orthogonal toggle (not part of
...
impl HardForkConfig {
    ...
    pub fn iter(&self) -> impl Iterator<Item = (&'static str, Option<u64>)> {
        [
            ...
            ("Espresso Enforcement", self.espresso_enforcement_time),
        ]
        .into_iter()
    }
}
...
mod tests {
    ...
    fn test_hardforks_iter() {
        let hardforks = HardForkConfig {
            ...
            espresso_enforcement_time: Some(12),
        };
        ...
        assert_eq!(iter.next(), Some(("Espresso Enforcement", Some(12))));
