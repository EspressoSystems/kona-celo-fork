...
pub trait DataAvailabilityProvider {
    ...
    /// event-only batch authorization enforcement) read it from `block_ref.timestamp` (the
    /// L1 origin time of the block being scanned), mirroring the `ecotoneTime` precedent.
    async fn next(
        &mut self,
        block_ref: &BlockInfo,
